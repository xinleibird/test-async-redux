...
import { queryScrapAndRepairList } from '../../actions';
...
const ScrapAndRepairList = () => {
  const scrapAndRepairList = useSelector(({ user }) => {
    return user.scrapAndRepairList;
  });

  const { data } = scrapAndRepairList;

  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(queryScrapAndRepairList());
  }, [dispatch]);
...
export default ScrapAndRepairList;
